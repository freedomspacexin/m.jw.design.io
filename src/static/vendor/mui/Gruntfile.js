...
module.exports = function(grunt) {
	...
	grunt.initConfig({
		...
		sass: {
			...
			dist: {
				files: {
					'<%= meta.distPath %>css/<%= pkg.name %>.css': '18_sass/mui.scss',
				}
			}
		},
		...
		cssmin: {
			options: {
				...
				keepSpecialComments: '*', // set to '*' because we already add the banner in 18_sass
				sourceMap: false
			},
			mui: {
